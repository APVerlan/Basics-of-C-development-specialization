...
public:
	...
	explicit Synchronized(T initial = T()) : _value(move(initial)) {};

	Accessor GetAccess() { return { _value, lock_guard(_m) }; }
	ConstAccessor GetAccess() const { return { _value, lock_guard(_m) }; };

...
public:
  ...
  struct ReadAccess {
	...
	ReadAccess(const Bucket& bucket, const K& key)
		: guard(bucket._m)
		, ref_to_value(bucket._data.at(key))
	{}
  };

  explicit ConcurrentMap(size_t bucket_count) : _num_buckets(bucket_count), 
	  _buckets(vector<Bucket>(bucket_count)) {
  }

  WriteAccess operator[](const K& key) {
	  size_t bucket_id = _hasher(key) % _num_buckets;

	  if (Has(key)) {
		  return { _buckets[bucket_id], key };
	  }
	  else return { _buckets[bucket_id], key };
  }
  ...
  bool Has(const K& key) const {
	  ...
	  lock_guard g(_buckets[bucket_id]._m);
	  return _buckets.at(bucket_id)._data.count(key);
  };

  MapType BuildOrdinaryMap() const {
	  MapType result_map;
	  for (size_t i = 0; i < _num_buckets; ++i) {
		  lock_guard g(_buckets[i]._m);
		  result_map.insert(_buckets[i]._data.begin(), _buckets[i]._data.end());
	  }

...
private:
	...
	public:
		ProxyCoeff(std::vector<T>& poly, size_t degree) : data_(poly), degree_(degree) {};

		void operator=(const T& val) {
			if (degree_ > data_.size() - 1) {
				vector<T> coeffs_new(degree_ + 1);
				copy(data_.begin(), data_.end(), coeffs_new.begin());
				data_ = move(coeffs_new);
				data_[degree_] = val;
		...
		operator T() const {
			...
			return std::as_const(data_)[degree_];
		}

		void SetDegree(size_t degree) { degree_ = degree; }
	};
private:

  std::vector<T> coeffs_ = {0};
  ProxyCoeff proxy_coeff_ = {coeffs_, 0};
...
public:
  ...
  ProxyCoeff& operator [](size_t degree) {
	  proxy_coeff_.SetDegree(degree);
	  return proxy_coeff_;
  }

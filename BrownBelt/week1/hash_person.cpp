...
#define STR_HASH(s) hash<string>{}(s)
#define INT_HASH(i) hash<int>{}(i)
#define DOUBLE_HASH(d) hash<double>{}(d)
...
struct Person {
  ...
  bool operator==(const Person& other) const {
    // реализуйте оператор
		return (tie(name, height, weight) == tie(other.name, other.height, other.weight)) 
			&& (address == other.address);
  }
};

template<typename T>
size_t POW(const T& value, const size_t i) {
	return (size_t)(pow(value, i));
};
...
struct AddressHasher {
  // реализуйте структуру
	...
	size_t operator() (const Address& adr) const {
		return (POW(x, 2) * STR_HASH(adr.city) 
				+ POW(x, 1) * STR_HASH(adr.street) + INT_HASH(adr.building));
	}
};
...
struct PersonHasher {
	...
	size_t operator() (const Person& pn) const {
		return POW(x, 3) * STR_HASH(pn.name) + POW(x, 2) * INT_HASH(pn.height) 
				+ x * DOUBLE_HASH(pn.weight) + AddressHasher{}(pn.address);
	} 
};

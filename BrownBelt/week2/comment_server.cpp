...
public:
	...
	HttpResponse& AddHeader(string name, string value) { 
		headers_.push_back(make_pair(name, value));
		return *this;
	};
	...
	friend ostream& operator << (ostream& output, const HttpResponse& resp) {
		...
		if (resp.code_ == HttpCode::Ok) output << "OK\n";
		else if (resp.code_ == HttpCode::Found) output << "Found\n";
		else if (resp.code_ == HttpCode::NotFound) output << "Not found\n";

		for (auto p : resp.headers_) {
    ...
    if (req.method == "POST") {
      if (req.path == "/add_user") {
        ...
        if (auto [id, response] = ParseIdAndContent(req.body); response == "42") {
		  ...
		  return HttpResponse(HttpCode::Ok);
        } else return HttpResponse(HttpCode::Found).AddHeader("Location", "/captcha");
      } else {
      ...
      if (req.path == "/user_comments") {
		...
		return HttpResponse(HttpCode::Ok).SetContent(response);
		  ...
		  string s = "What's the answer for The Ultimate Question of Life, the Universe, and Everything?";
		  return HttpResponse(HttpCode::Ok).SetContent(s);
      } else {
		  return HttpResponse(HttpCode::NotFound);
      }
    }
